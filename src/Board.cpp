...
void Board::CalcMIN_SEPARATION()	// Sets m_dMinSeparation and m_warnPoints[]
{
	...
	const int nRings = ( m_compMgr.GetNumPlacedSOIC() > 0 ) ? 3 : 2;	// 2 ==> Max pad size supported by VeroRoute could be up to 200 mil in future
		...
		if ( bMatch )
		{
			...
			if ( pC->GetHasPinTH() )
			{
				...
				else
				{
					size_t	pinIndex;
					int		compId;
					GetSlotInfoForTH(pC, pinIndex, compId);
			...
			if ( pR->GetHasPinTH() )
			{
				...
				else
				{
					size_t	pinIndex;
					int		compId;
					GetSlotInfoForTH(pR, pinIndex, compId);
